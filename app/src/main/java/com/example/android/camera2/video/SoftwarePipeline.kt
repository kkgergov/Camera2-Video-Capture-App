...
class SoftwarePipeline(width: Int, height: Int, fps: Int,
        characteristics: CameraCharacteristics, encoder: EncoderWrapper,
        viewFinder: AutoFitSurfaceView) : Pipeline(width, height, fps,
                characteristics, encoder, viewFinder) {

    override fun createPreviewRequest(session: CameraCaptureSession): CaptureRequest? {
    ...
    override fun createRecordRequest(session: CameraCaptureSession): CaptureRequest {
        // Capture request holds references to target surfaces
